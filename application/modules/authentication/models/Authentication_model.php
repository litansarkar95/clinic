...
class Authentication_model extends CI_Model
{
    ...
    public function getUserNameByRoleID($roleID, $userID = '')
    {
        ...
        if ($roleID == 7) {
            ...
            $sql = "SELECT customers.id, customers.name as name, customers.email, customers.contact_no, customers.picture FROM customers  WHERE customers.id = " . $this->db->escape($userID);
            return $this->db->query($sql)->row_array();
        } else {

...
public function save_bill()
{
    ...
    $original_price = 0; // initialize variable
    $total = 0;

    foreach ($cart as $item) {
        // Make sure 'original_price' exists in $item
        $price = isset($item['original_price']) ? $item['original_price'] : 0;
    ...
    //Send SMS
  // link
    $link = $this->generate($billing_id);
     // end sms template

           if ($original_price > $grand_total) {

            $discount = $original_price - $grand_total;
            $message = "Dear $name, ,Your TOTAL Bill Was Tk $original_price. After a DISCOUNT of $discount, FINAL AMOUNT: $grand_total. Payment RECEIVED Successfully.View your invoice: $link -ZAREEN'S BEAUTY SALON";

        } else {

            $message ="DEAR $name,Your TOTAL Bill Was Tk $grand_total. Payment Received Successfully. View your invoice: $link -ZAREEN'S BEAUTY SALON";

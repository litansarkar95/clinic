...
public function index()
{
  ...
  $this->form_validation->set_rules("name", display('name'), "required|is_unique[departments.name]");
  ...
  if ($this->form_validation->run() == NULL) {
    ...
    $data = array(   
       ...
       // "commission"                 => $this->common_model->xss_clean($this->input->post("commission")),   
        "imap_username"              => $this->common_model->xss_clean($this->input->post("imap_username")),
        "email"                      => $this->common_model->xss_clean($this->input->post("email")),
        "host"                       => $this->common_model->xss_clean($this->input->post("host")),
        "password"                   => $this->common_model->xss_clean($this->input->post("password")),
        "encryption"                 => $this->common_model->xss_clean($this->input->post("encryption")),
        "hidefromclient"             => $this->common_model->xss_clean($this->input->post("hidefromclient")),
        "is_active"                  => 1,
        "create_date"                => strtotime($date),
       
    );

    if ($this->common_model->save_data("departments", $data)) {
   ...
   redirect(base_url() . "staff/departments", "refresh");
  }


  $data = array();
  $data['active'] = "departments";
  $data['title'] = display('create')." ".display('departments'); 
  $data['allPdt'] =  $this->common_model->view_data("departments", "", "id", "asc");
 
  $data['content'] = $this->load->view("staff/departments", $data, TRUE);
 $this->load->view('layout/master', $data);
}

public function delete($id) {
   
    $dt = $this->common_model->view_data("departments", array("id" => $id), "id", "asc");
   
   
    if ($dt) {
   
        $this->common_model->delete_data("departments", array("id" => $id));

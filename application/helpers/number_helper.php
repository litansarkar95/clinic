...
if (!function_exists('number_to_words')) {
    function number_to_words($num) {
        ...
        // Handle decimal part (if any) for Paisa
        $decimal_part = round(($num - floor($num)) * 100); // Get the decimal part as a whole number (e.g., 20 for .20)
        
        if ($decimal_part > 0) {
            $result .= " Taka " . $convert_to_words($decimal_part) . " Paisa";
        } else {
            $result .= " Taka";
        }
